...
- **极简主义** — 93 行代码，每行都有中文注释，初学者也能看懂

---

## LumiAI-Ecosystem

LumiAgent 是 LumiAI-Ecosystem 的调度核心，负责 AI 编程辅助任务。生态内的其他子项目：

| 项目 | 定位 | 说明 |
|:---|:---|:---|
| **LumiAgent** | 调度核心 | 为老旧设备而生的 AI 编程调度器 |
| **LumiBrain** | 记忆中枢 | 对话历史管理与上下文记忆 |
| **LumiData** | 数据中心 | 本地知识库与数据持久化 |
| **LumiSkills** | 技能插件 | 可插拔的领域能力扩展 |
...
所以我用最直接的方式——调用 Ollama API，用 TRAE IDE 辅助开发，写了 93 行 Python——做出了一套给老旧设备用的 AI 编程助手。
...
```
lumiagent/
├── agent_core.py    # 核心代码（93行，含中文注释）
├── demo.html        # 项目展示页（含运行环境说明）
...
| 方向 | 说明 |
|:---|:---|
| 双模型调度 | 本地守门人判断 → 简单留本地 → 复杂升云端 |
| 内容自检 | 检测模型回答中的技术错误，提醒用户保持警惕 |
| 上下文记忆 | 记住最近对话，让模型理解上下文 |
...
&copy; 2026 Lumi by Kai. MIT License.
